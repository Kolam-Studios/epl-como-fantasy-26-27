...
| Real roster on Vercel | `LEAGUE_CONFIG_LOCAL` env var holding the JSON override | Vercel builds from the public repo and can't see the gitignored `league.config.local.json`; the config loader needs a small change to read this env var (tracked as an issue) |

## Accounts checklist

| Account | Status | Owner | Notes |
|---|---|---|---|
| GitHub | ✅ exists | deployment owner | Repo owner must add the deployment owner's GitHub account as a **collaborator** (repo → Settings → Collaborators) so Vercel can watch the repo |
...
| Vercel (vercel.com) | ⬜ to create | deployment owner | Sign up **with the GitHub account** ("Continue with GitHub") — this is what lets Vercel see and deploy the repo. Choose Hobby (free) |
...
1. **Collaborator access** — repo owner adds the deployment owner's GitHub account as a collaborator. (5 min, repo owner)
2. **Create the Vercel account** — vercel.com → "Continue with GitHub" → Hobby plan. (5 min)
...
1. **Roster env override** — code change to the config loader (issue filed; small, must land before the production deploy is real).
