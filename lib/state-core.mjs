...
import { openBidFor, minOpenBid } from "./config-core.mjs";
import { listPeriods } from "./period-core.mjs";
import {
 ...
 * guard at the read layer - the reveal stays up even if the band is missing.
 */
...
export async function buildStatePayload(sql, cfg) {
  const [appStateRows, managerRows, soldRows, spendRows, moveRows, tradeRows, nameRows, fairBand] = await Promise.all([
    sql`select * from app_state where id = 1`,
    sql`select id, slot, short, sat_out from managers order by slot`,
    // The ONLY query that touches valuations: rooted in ACTIVE sales (a
    // dropped player is a free agent again, so their value RE-SEALS), so
    // values can only appear for players that are currently sold.
    sql`
    ...
    `,
    // EVERY sales row (active + released): spend is sunk and never refunded,
    // so the money maths always sums the full history.
    sql`select manager_id, price from sales`,
    ...
    // so every board number is post-trade correct. Movements are joined to
    // the ACTIVE sale and clipped to trades at/after it, so a previous
    // incarnation's trades cannot move a re-signed player. No valuations here.
    sql`
      select tp.player_id, tp.from_manager, tp.to_manager, t.id as seq
      from trade_players tp
      join trades t on t.id = tp.trade_id
      join sales s on s.player_id = tp.player_id and s.released = false
      where t.voided = false and t.created_at >= s.created_at
      order by t.id
    `,
    sql`
    ...
    `,
    // Whole-pool name universe (#44): the id/web_name/team_short of every
    // player, so a shared surname is disambiguated the same way here as in the
    // ledger. Names only - no valuations, no sealing concern.
    sql`select id, web_name, team_short from players`,
    // Draft-morning calibrated reveal band (#70); null falls back to config.
    readFairBand(sql),
  ]);

  // Disambiguated on-screen names (#44). Every name-bearing field below reads
  // its label from here; the raw web_name still rides alongside as `name`.
  ...
  const saleRows = soldRows.map((r) => ({
    playerId: r.player_id,
    managerId: r.manager_id,
    price: r.price,
    position: r.position,
  }));
  const ownership = resolveOwnership(saleRows, movements);
  const auctionByManager = auctionSpendByManager(
    spendRows.map((r) => ({ managerId: r.manager_id, price: r.price })),
  );
  ...
  const derived = deriveManagers(cfg, managerRows, ownership, cashByManager, auctionByManager);
  ...
  const satOutById = new Map(managerRows.map((r) => [r.id, r.sat_out === true]));
  const managers = derived.map((m) => ({
    id: m.managerId,
    slot: m.slot,
    short: m.short,
    // Sat-out managers (owner ruling) are greyed/skipped by writer surfaces;
    // their derived numbers still render everywhere as the record.
    satOut: satOutById.get(m.managerId) ?? false,
    spent: m.spent,
    ...
    squad: ownership
      ...
      .map((o) => {
        ...
        return {
          playerId: o.playerId,
          name: d?.name ?? null,
          displayName: displayNameById.get(o.playerId) ?? d?.name ?? null,
          position: o.position,
          tier: d?.tier ?? null,
          price: o.price,
        };
  ...
  const recentTrades = tradeRows.slice(0, RECENT_TRADES).map((t) => ({
    ...
    players: (movesByTrade.get(t.id) || []).map((mv) => ({
      ...
      displayName: displayNameById.get(mv.player_id) ?? soldByPlayer.get(mv.player_id)?.name ?? null,
  ...
  if (lot) {
    currentLot = {
      ...
      code: lot.code,
      name: lot.web_name,
      displayName: displayNameById.get(lot.id) ?? lot.web_name,
  ...
  const recentSales = soldRows.slice(0, RECENT_SALES).map((r) => {
    const v = saleVerdict(cfg, r.price, r.value, fairBand);
    return {
      saleId: r.sale_id,
      playerId: r.player_id,
      playerName: r.web_name,
      displayName: displayNameById.get(r.player_id) ?? r.web_name,
      code: r.code,
      position: r.position,
      tier: r.tier,
  ...
  if (last) {
    const v = saleVerdict(cfg, last.price, last.value, fairBand);
    reveal = {
      saleId: last.sale_id,
      playerId: last.player_id,
      playerName: last.web_name,
      displayName: displayNameById.get(last.player_id) ?? last.web_name,
  ...
  // --- season periods (waiver era) -------------------------------------
  // The phase rail needs every period with its status; the live surfaces
  // need which one is current. Pre-backfill (no periods seeded, or no
  // current pointer) this is an empty list / null and every consumer keeps
  // the auction-era behavior.
  const periods = await listPeriods(sql);
  const currentPeriodId = appState?.current_period_id ?? null;
