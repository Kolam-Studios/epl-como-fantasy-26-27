...
import { minOpenBid, squadSize, walletBudget } from "./config-core.mjs";
...
/**
 * Sum of each manager's OWN auction purchases - their sunk market spend. Keyed
 * by the buyer on the sales row (the original hammer buyer; sales rows keep that
 * manager_id even after the player is traded away). This money is spent on the
 * market and is NEVER refunded by a later trade - a trade moves the player and
 * settles cash, but the buyer's original outlay stays spent. deriveManager adds
 * net trade cash on top of this to get a manager's true spend.
 *
 * @param {Array<{managerId: number, price: number}>} saleRows
 * @returns {Record<number, number>}
 */
export function auctionSpendByManager(saleRows) {
  /** @type {Record<number, number>} */
  const out = {};
  for (const s of saleRows) out[s.managerId] = (out[s.managerId] || 0) + s.price;
  return out;
}
 ...
 *
 * Spend is the manager's OWN auction purchases (`auctionSpend`, sunk on the
 * market - see auctionSpendByManager) plus net trade cash (`cashOut`). It is
 * NOT the sum of currently-owned salaries: trading a player away does not refund
 * what you paid for him at the hammer; that money is spent on the market. Squad
 * slots and position fills, by contrast, DO come from current ownership (`owned`,
 * post-trade) - so a player traded to you fills a slot without changing what you
 * spent, and one traded away frees a slot without a refund.
 *
 ...
 * remaining.
 *
 * Remaining is measured against the SEASON wallet (`walletBudget`: the auction
 * budget plus any post-auction top-up the owners have granted), not against the
 * auction budget alone.
 *
 * @param {LeagueConfig} cfg
 * @param {OwnedPlayer[]} owned  current holdings (for slots/fills only)
 * @param {number} [cashOut]     net trade cash paid out (spend terms), default 0
 * @param {number} [auctionSpend] this manager's own auction purchases, default 0
 * @returns {ManagerDerived}
 */
export function deriveManager(cfg, owned, cashOut = 0, auctionSpend = 0) {
  const spent = auctionSpend + cashOut;
  const remaining = walletBudget(cfg) - spent;
 ...
 * @param {Record<number, number>} [auctionByManager]  output of auctionSpendByManager
 * @returns {Array<{managerId: number, slot: number, short: string} & ManagerDerived>}
 */
export function deriveManagers(cfg, managers, ownership, cashByManager = {}, auctionByManager = {}) {
  return managers.map((m) => {
    const owned = ownership.filter((o) => o.managerId === m.id);
    const cashOut = cashByManager[m.id] || 0;
    const auctionSpend = auctionByManager[m.id] || 0;
    return { managerId: m.id, slot: m.slot, short: m.short, ...deriveManager(cfg, owned, cashOut, auctionSpend) };
  });
}
...
/**
 * Verdict logic for the post-sale reveal:
 * delta = price - value; FAIR when |delta| <= the band, otherwise OVERPAY when
 * price > value, STEAL when price < value. pctOver = delta / value (how far
 * over the sealed value the hammer landed; negative = under). Null when the
 * value is 0 or missing.
 *
 * The FAIR band is calibrated on draft morning from the day's valuations and
 * passed in as `fairBand` (#70). config.valueBadgeThreshold is ONLY the
 * graceful fallback used when no calibrated band is available (the valuations
 * job has not run) - the AI must never block or distort the auction, so a
 * missing band degrades to the config number rather than breaking the verdict.
 * A `fairBand` of exactly 0 is honoured (only an exact hit reads FAIR).
 *
 * @param {LeagueConfig} cfg
 * @param {number} price
 * @param {number | null | undefined} value
 * @param {number | null} [fairBand]  calibrated band in dollars; falls back to
 *                                     config.valueBadgeThreshold when not a
 *                                     finite number >= 0
 * @returns {{delta: number | null, pctOver: number | null, verdict: "OVERPAY" | "FAIR" | "STEAL" | null}}
 */
export function saleVerdict(cfg, price, value, fairBand) {
  if (value == null) return { delta: null, pctOver: null, verdict: null };
  const band =
    typeof fairBand === "number" && Number.isFinite(fairBand) && fairBand >= 0
      ? fairBand
      : cfg.valueBadgeThreshold;
