...
/**
 * Resolve current ownership from raw rows.
 *
 * Sales rows are DELETED on void (see db/schema.sql), so "sales minus voided"
 * is simply: every sales row that exists is a current ownership, and the sale
 * price is the salary the player carries.
 *
 * SEAM (S3, trades): when trades land, fold trade movements in here - for
 * each player, the latest non-voided trade movement overrides the sale's
 * manager (the salary still travels with the player), and net trade cash is
 * folded into spend separately. Until then the trades tables are empty and
 * passing any movements is an error so the seam cannot be silently misused.
 *
 * @param {Array<{playerId: number, managerId: number, price: number, position: Position}>} saleRows
 * @param {Array<unknown>} [tradeMovements]
 * @returns {OwnedPlayer[]}
 */
export function resolveOwnership(saleRows, tradeMovements = []) {
  if (tradeMovements.length > 0) {
    throw new Error(
      "Trade movements are not implemented yet (S3). resolveOwnership only handles sales.",
    );
  }
 ...
 */
export function deriveManager(cfg, owned) {
  const spent = owned.reduce((sum, p) => sum + p.price, 0);
 ...
 */
export function deriveManagers(cfg, managers, ownership) {
  return managers.map((m) => {
    const owned = ownership.filter((o) => o.managerId === m.id);
    return { managerId: m.id, slot: m.slot, short: m.short, ...deriveManager(cfg, owned) };
  });
}
