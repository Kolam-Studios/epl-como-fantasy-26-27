// Sale corrections (undo last / edit any / void any) - the commissioner's
// fix-it tools per 02-SPEC §E/§F and 04-DATA-MODEL. Plain JS (with JSDoc
// types) so scripts/test-corrections.mjs can drive the exact same
// transactions the routes serve, without a build step. lib/corrections.ts
// wraps these with the app's shared pool and config.
//
// EVERY mutation here runs inside withAuctionLock (lib/draft-core.mjs): the
// FOR UPDATE on the app_state singleton serialises all auction writes, so a
// correction can never interleave with a sale (see the comment on app_state
// in db/schema.sql). Every mutation writes an audit row and bumps
// app_state.version; rule violations return structured rejections
// ({ok:false, code, message}) and never write anything (the transaction only
// performs writes after all checks pass).

import { minOpenBid, openBidFor, squadSize } from "./config-core.mjs";
import { withAuctionLock } from "./draft-core.mjs";
import { waiverBlackout } from "./period-core.mjs";
import { auctionSpendByManager, deriveManager, isEligible, resolveOwnership, tradeCashByManager } from "./derive-core.mjs";
import { loadOwnershipContext } from "./ownership-core.mjs";

/** @typedef {import("./config-core.mjs").LeagueConfig} LeagueConfig */

/**
 * @typedef {Object} CorrectionRejection
 * @property {false} ok
 * @property {string} code    machine-readable rule name
 * @property {string} message plain-English rule + number, console-displayable
 */

/**
 * @typedef {Object} SaleSnapshot  a sale row as it stood (audit payload shape)
 * @property {number} id
 * @property {number} player_id
 * @property {number} manager_id
 * @property {number} price
 * @property {number | null} lot_no
 * @property {number | null} phase
 * @property {string} created_at   ISO instant
 */

/** @param {string} code @param {string} message @returns {CorrectionRejection} */
function reject(code, message) {
  return { ok: false, code, message };
}

/** @param {unknown} reason */
function missingReason(reason) {
  return typeof reason !== "string" || reason.trim() === "";
}

/**
 * The name of the player if it has ANY non-voided trade movement, else null.
 *
 * A sale correction (undo / edit / void) is illegal while the sale's player is
 * tied up in a trade. Corrections remove or re-point the sales ROW, but there
 * is no trade-void in v1, so the player's trade_players movements would be left
 * dangling. resolveOwnership ignores a movement whose player has no sale row -
 * harmless while the player is unsold, but the moment that player id gets a
 * fresh sales row (a re-auction), the stale movement re-attaches and overrides
 * the new owner: the player lands on the wrong manager, the real buyer is
 * mischarged, and an uninvolved third manager can be pushed negative or over
 * quota (recordSale validates only the buyer). Blocking corrections on a traded
 * player closes every one of those paths at the source, since void/undo/
 * edit-change-player are the ONLY ways a sold player becomes unsold.
 *
 * The recourse is to correct sales BEFORE trading their players; unwinding a
 * trade after the fact is a v2 trade-void feature.
 *
 * @param {import("postgres").TransactionSql} tx
 * @param {number} playerId
 * @returns {Promise<string | null>}
 */
async function tradedPlayerName(tx, playerId) {
  const [row] = await tx`
    select p.web_name
    from trade_players tp
    join trades t on t.id = tp.trade_id
    join players p on p.id = tp.player_id
    where tp.player_id = ${playerId} and t.voided = false
    limit 1
  `;
  return row ? row.web_name : null;
}

/**
 * Snapshot a sales row into the audit payload shape (snake_case row fields,
 * created_at as an ISO string so before/after JSON is stable and comparable).
 *
 * @param {any} row @returns {SaleSnapshot}
 */
function snapshot(row) {
  return {
    id: row.id,
    player_id: row.player_id,
    manager_id: row.manager_id,
    price: row.price,
    lot_no: row.lot_no,
    phase: row.phase,
    created_at: new Date(row.created_at).toISOString(),
  };
}

/**
 * Undo the MOST RECENT sale (by created_at, tiebreak id): delete the row,
 * write a 'sale.void' audit entry (reason "undo last sale", before = the
 * deleted row), and restore the board - the undone player goes straight back
 * on the block (current_player_id), tv_view returns to 'block' and any
 * pending reveal is cancelled (reveal_until = null).
 *
 * QUEUE RESTORATION (verified against draft-core.mjs): the lot advance in
 * recordSale is DERIVED, not stored - after a sale it scans
 * lot_queue.slice(queueIdx + 1) for the first player NOT in the sold set
 * (a fresh `select player_id from sales`). So putting current_player_id back
 * to the undone player is enough: the queue itself was never mutated, the
 * deleted row leaves the sold set, and when the restored lot sells again the
 * same scan lands on the same next player as before (still unsold). The
 * previously-advanced lot simply becomes "next" again. Phase-2 lots are not
 * queue-driven at all, so restoring current_player_id covers them too.
 *
 * DOUBLE-SUBMIT GUARD: pass expectedSaleId (the sale id the caller believes
 * is newest) and the undo only proceeds when the actual newest sale matches;
 * otherwise it rejects with 'stale_undo' and writes NOTHING. Two rapid
 * clicks of "undo" then remove one sale, not two: the first deletes it, the
 * second sees a different (or no) newest sale and bounces. When
 * expectedSaleId is absent the old unconditional behaviour is kept (API
 * compatibility).
 *
 * @param {import("postgres").Sql} sql  a postgres.js client (pool)
 * @param {LeagueConfig} cfg            unused today; kept for signature parity
 * @param {{actor: string, expectedSaleId?: number}} args
 * @returns {Promise<{ok: true, undone: SaleSnapshot} | CorrectionRejection>}
 */
export async function undoLastSale(sql, cfg, { actor, expectedSaleId }) {
  return await withAuctionLock(sql, async (tx, appState) => {
    if (!appState) {
      return reject(
        "no_state",
        "The draft has not been initialised (app_state is empty). Run the seed first.",
      );
    }

    // The waiver blackout (docs/DESIGN-WAIVERS.md 2.3/4.4) holds corrections
    // too: a void or edit between a cutoff and its published results would
    // change the budgets and rosters the forms were final against.
    {
      const blackout = await waiverBlackout(tx, appState);
      if (blackout.paused) {
        return reject(
          "corrections_paused",
          `Sale corrections are paused while ${blackout.label} resolves; they reopen when the results publish.`,
        );
      }
    }

    const [last] = await tx`
      select id, player_id, manager_id, price, lot_no, phase, created_at
      from sales
      where released = false
      order by created_at desc, id desc
      limit 1
    `;
    if (!last) {
      return reject("nothing_to_undo", "No sale to undo.");
    }
    if (expectedSaleId !== undefined && last.id !== expectedSaleId) {
      return reject("stale_undo", "The last sale changed - refresh and try again.");
    }

    const tradedName = await tradedPlayerName(tx, last.player_id);
    if (tradedName) {
      return reject(
        "player_traded",
        `${String(tradedName).toUpperCase()} has been traded since this sale - reverse the trade before undoing this sale (trade voiding is a v2 feature).`,
      );
    }

    const before = snapshot(last);
    await tx`delete from sales where id = ${last.id}`;

    await tx`
      insert into audit_log (actor, action, entity, entity_id, before, after, reason)
      values (${actor}, 'sale.void', 'sale', ${last.id}, ${tx.json(before)}, ${null},
              'undo last sale')
    `;

    // The natural behaviour the room expects: the player goes back on the
    // block, and the TV drops any in-flight reveal of the now-undone sale.
    await tx`
      update app_state
      set current_player_id = ${last.player_id},
          tv_view = 'block',
          reveal_until = null,
          version = version + 1
      where id = 1
    `;

    return { ok: true, undone: before };
  });
}

/**
 * Edit any sale: change its player, manager and/or price, re-validating the
 * FULL legality of the sale in the new state. The buying manager's derived
 * numbers (quota, squad size, max bid) are recomputed EXCLUDING this sale
 * itself - otherwise the sale's own current price would be double-counted in
 * spend when raising the price, and its own slot would wrongly block a
 * same-manager edit. reason is REQUIRED, and at least one of
 * playerId/managerId/price must be provided (else 'nothing_to_change').
 *
 * PLAYER CHANGES (the most common live mistake is hammering the wrong
 * player): when playerId is given, the new player must exist, must not be
 * the player currently ON THE BLOCK (player_on_block - marking the live lot
 * as sold would strand a sold player as the current lot), must not
 * already be sold to anyone (same already_sold shape as recordSale, naming
 * the current owner and price), and EVERY check below runs against the NEW
 * player - the (possibly new) manager's position quota against the new
 * player's position (a manager full at FWD cannot have a sale edited onto
 * another FWD), the price floor against the new player's tier opening bid.
 * The max-bid maths is unchanged: it still excludes this sale's own price.
 *
 * Does not touch current_player_id or tv_view: editing an old sale does not
 * change what's on the block.
 *
 * @param {import("postgres").Sql} sql  a postgres.js client (pool)
 * @param {LeagueConfig} cfg
 * @param {{saleId: number, playerId?: number, managerId?: number, price?: number, reason: string, actor: string}} args
 * @returns {Promise<{ok: true, sale: SaleSnapshot, before: SaleSnapshot} | CorrectionRejection>}
 */
export async function editSale(sql, cfg, { saleId, playerId, managerId, price, reason, actor }) {
  if (missingReason(reason)) {
    return reject("missing_reason", "A reason is required to edit a sale.");
  }
  if (playerId === undefined && managerId === undefined && price === undefined) {
    return reject("nothing_to_change", "Nothing to change.");
  }
  try {
    return await withAuctionLock(sql, async (tx, appState) => {
      if (!appState) {
        return reject(
          "no_state",
          "The draft has not been initialised (app_state is empty). Run the seed first.",
        );
      }

      // Blackout holds for edits too (see the undo/void guards above).
      {
        const blackout = await waiverBlackout(tx, appState);
        if (blackout.paused) {
          return reject(
            "corrections_paused",
            `Sale corrections are paused while ${blackout.label} resolves; they reopen when the results publish.`,
          );
        }
      }

      const [sale] = await tx`
        select id, player_id, manager_id, price, lot_no, phase, created_at
        from sales where id = ${saleId} and released = false
      `;
      if (!sale) {
        return reject("not_found", `Sale #${saleId} not found.`);
      }

      // The sale's player must not be tied up in a trade (see tradedPlayerName).
      const tradedName = await tradedPlayerName(tx, sale.player_id);
      if (tradedName) {
        return reject(
          "player_traded",
          `${String(tradedName).toUpperCase()} has been traded since this sale - reverse the trade before editing this sale (trade voiding is a v2 feature).`,
        );
      }

      const newPlayerId = playerId ?? sale.player_id;
      const newManagerId = managerId ?? sale.manager_id;
      const newPrice = price ?? sale.price;

      // All checks below run against the NEW player (position quota, tier
      // opening bid). When playerId is not being changed this is the sale's own
      // player (FK-guaranteed to exist; belt and braces).
      const [player] = await tx`
        select id, web_name, position, tier from players where id = ${newPlayerId}
      `;
      if (!player) {
        return reject("unknown_player", `No player with id ${newPlayerId} exists in the pool.`);
      }

      if (newPlayerId !== sale.player_id) {
        // The player currently ON THE BLOCK cannot be edited onto a sale:
        // the row would mark them sold while app_state still shows them as
        // the current lot - a sold player stranded on the block. Resolve the
        // lot (hammer or no-bid) first.
        if (appState.current_player_id != null && newPlayerId === appState.current_player_id) {
          return reject(
            "player_on_block",
            `${String(player.web_name).toUpperCase()} is on the block right now - resolve the lot first.`,
          );
        }

        // Exclusive ownership: the new player must not be sold to ANYONE. This
        // sale's own row is on the old player, but exclude it by id anyway so
        // the check stays correct whatever the row states.
        const [existing] = await tx`
          select s.price, m.short
          from sales s join managers m on m.id = s.manager_id
          where s.player_id = ${newPlayerId} and s.id <> ${saleId} and s.released = false
        `;
        if (existing) {
          return reject(
            "already_sold",
            `${String(player.web_name).toUpperCase()} is already sold to ${existing.short} for $${existing.price}.`,
          );
        }
      }

      const [manager] = await tx`
        select id, slot, short from managers where id = ${newManagerId}
      `;
      if (!manager) {
        return reject("unknown_manager", `No manager with id ${newManagerId} exists.`);
      }

      // The buying manager's squad EXCLUDING the sale being edited: the edit
      // replaces this sale wholesale, so its current price must not count in
      // spend and its slot must not count as filled (else lowering-then-raising
      // a price double-counts, and a same-manager edit can never pass a full
      // quota it itself fills). Resolved GLOBALLY through trades so a
      // traded-in player and trade cash count toward the buyer's true squad;
      // dropping the edited sale row also drops any trade movement of its
      // player (that sale is being replaced). With no trades this is identical
      // to the old per-manager, exclude-self read.
      const ctx = await loadOwnershipContext(tx);
      const salesExclSelf = ctx.sales.filter((s) => s.saleId !== saleId);
      const ownership = resolveOwnership(salesExclSelf, ctx.movements);
      const cash = tradeCashByManager(ctx.trades);
      // Auction spend excludes the edited sale too - it is being re-validated as
      // if it were not yet recorded (its sunk cost re-applies at the new price).
      const auction = auctionSpendByManager(salesExclSelf);
      const derived = deriveManager(
        cfg,
        ownership.filter((o) => o.managerId === newManagerId),
        cash[newManagerId] || 0,
        auction[newManagerId] || 0,
      );

      const size = squadSize(cfg);
      if (derived.squadComplete) {
        return reject(
          "squad_complete",
          `${manager.short} has a complete squad (${size}/${size}).`,
        );
      }
      if (!isEligible(cfg, derived, player.position)) {
        const quota = cfg.squad[player.position];
        return reject(
          "position_full",
          `${manager.short} has no open ${player.position} slot (${derived.fills[player.position]}/${quota}).`,
        );
      }

      if (typeof newPrice !== "number" || !Number.isInteger(newPrice) || newPrice <= 0) {
        return reject(
          "bad_price",
          `Price must be a positive whole number of dollars (got ${JSON.stringify(newPrice)}).`,
        );
      }
      // Re-validate against the SALE's own phase: a phase-2 sale opens at the
      // league-wide minimum ($1) regardless of tier, matching recordSale.
      const openBid =
        sale.phase === 2
          ? minOpenBid(cfg)
          : player.tier != null
            ? openBidFor(cfg, player.tier)
            : minOpenBid(cfg);
      if (newPrice < openBid) {
        return reject(
          "below_open",
          sale.phase === 2
            ? `Below the phase-2 minimum bid ($${openBid}).`
            : `Below the Tier ${player.tier ?? "?"} opening bid ($${openBid}).`,
        );
      }
      if (derived.maxBid != null && newPrice > derived.maxBid) {
        return reject(
          "over_max_bid",
          `Over ${manager.short}'s max bid of $${derived.maxBid} - they must keep $${minOpenBid(cfg)} per open slot. Rejected.`,
        );
      }

      const before = snapshot(sale);
      const [updated] = await tx`
        update sales
        set player_id = ${newPlayerId}, manager_id = ${newManagerId}, price = ${newPrice}
        where id = ${saleId}
        returning id, player_id, manager_id, price, lot_no, phase, created_at
      `;
      const after = snapshot(updated);

      await tx`
        insert into audit_log (actor, action, entity, entity_id, before, after, reason)
        values (${actor}, 'sale.edit', 'sale', ${saleId},
                ${tx.json(before)}, ${tx.json(after)}, ${reason.trim()})
      `;

      await tx`update app_state set version = version + 1 where id = 1`;

      return { ok: true, sale: after, before };
    });
  } catch (err) {
    // The UNIQUE(player_id) backstop on the UPDATE, same shape as recordSale:
    // all writers should serialise on the auction lock, but if a non-lock
    // writer ever races the already_sold pre-check, the constraint fires and
    // we return the same already-sold rejection instead of a 500. (playerId
    // is always set here: without a player change the UPDATE keeps the row's
    // own player_id and cannot conflict.)
    if (err && err.code === "23505") {
      const [existing] = await sql`
        select s.price, m.short, p.web_name
        from sales s
        join managers m on m.id = s.manager_id
        join players p on p.id = s.player_id
        where s.player_id = ${playerId} and s.released = false
      `;
      return reject(
        "already_sold",
        existing
          ? `${String(existing.web_name).toUpperCase()} is already sold to ${existing.short} for $${existing.price}.`
          : `Player ${playerId} is already sold.`,
      );
    }
    throw err;
  }
}

/**
 * Void any sale: delete the row (the player becomes available again - the
 * UNIQUE(player_id) constraint then allows a later re-sale), write a
 * 'sale.void' audit entry with the deleted row and the required reason, bump
 * the version. reason is REQUIRED.
 *
 * Deliberately does NOT touch current_player_id: voiding an old sale doesn't
 * change what's on the block. The semantics stay distinct from undoLastSale
 * on purpose (undo = "that hammer was wrong, put the player back up"; void =
 * "strike this sale from the record, the auction rolls on").
 *
 * ONE exception on tv_view: if the voided sale is the NEWEST sale (created_at
 * desc, id desc - the same ordering undoLastSale uses) AND the TV is mid
 * reveal, the reveal on screen is THIS sale's reveal. Leaving it running
 * would replay a now-voided sale to the room (and once it expires the TV
 * would fall back showing stale data), so we cancel it: tv_view = 'block',
 * reveal_until = null. Voiding any OLDER sale leaves the TV alone - the
 * running reveal belongs to a different, still-valid sale.
 *
 * @param {import("postgres").Sql} sql  a postgres.js client (pool)
 * @param {LeagueConfig} cfg            unused today; kept for signature parity
 * @param {{saleId: number, reason: string, actor: string}} args
 * @returns {Promise<{ok: true, voided: SaleSnapshot} | CorrectionRejection>}
 */
export async function voidSale(sql, cfg, { saleId, reason, actor }) {
  if (missingReason(reason)) {
    return reject("missing_reason", "A reason is required to void a sale.");
  }
  return await withAuctionLock(sql, async (tx, appState) => {
    if (!appState) {
      return reject(
        "no_state",
        "The draft has not been initialised (app_state is empty). Run the seed first.",
      );
    }

    // The waiver blackout (docs/DESIGN-WAIVERS.md 2.3/4.4) holds corrections
    // too: a void or edit between a cutoff and its published results would
    // change the budgets and rosters the forms were final against.
    {
      const blackout = await waiverBlackout(tx, appState);
      if (blackout.paused) {
        return reject(
          "corrections_paused",
          `Sale corrections are paused while ${blackout.label} resolves; they reopen when the results publish.`,
        );
      }
    }

    const [sale] = await tx`
      select id, player_id, manager_id, price, lot_no, phase, created_at
      from sales where id = ${saleId} and released = false
    `;
    if (!sale) {
      return reject("not_found", `Sale #${saleId} not found.`);
    }

    // The sale's player must not be tied up in a trade (see tradedPlayerName):
    // voiding it would strand the current owner and leave a dangling movement
    // that corrupts a later re-sale.
    const tradedName = await tradedPlayerName(tx, sale.player_id);
    if (tradedName) {
      return reject(
        "player_traded",
        `${String(tradedName).toUpperCase()} has been traded since this sale - reverse the trade before voiding this sale (trade voiding is a v2 feature).`,
      );
    }

    // Is this the newest sale? (Checked BEFORE the delete, same ordering as
    // undoLastSale.) If so and the TV is mid-reveal, that reveal is showing
    // THIS sale - clear it below so the room never sees a voided sale's
    // reveal keep playing (or the previous sale's reveal replay).
    const [newest] = await tx`
      select id from sales
      where released = false
      order by created_at desc, id desc
      limit 1
    `;
    const voidingRevealedSale =
      newest != null && newest.id === sale.id && appState.tv_view === "reveal";

    const before = snapshot(sale);
    await tx`delete from sales where id = ${saleId} and released = false`;

    await tx`
      insert into audit_log (actor, action, entity, entity_id, before, after, reason)
      values (${actor}, 'sale.void', 'sale', ${saleId},
              ${tx.json(before)}, ${null}, ${reason.trim()})
    `;

    if (voidingRevealedSale) {
      await tx`
        update app_state
        set tv_view = 'block', reveal_until = null, version = version + 1
        where id = 1
      `;
    } else {
      await tx`update app_state set version = version + 1 where id = 1`;
    }

    return { ok: true, voided: before };
  });
}
