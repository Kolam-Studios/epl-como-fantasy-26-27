...
import { currentPeriodStamp, waiverBlackout } from "./period-core.mjs";
...
export async function recordSale(sql, cfg, { playerId, managerId, price, actor }) {
  try {
    return await withAuctionLock(sql, async (tx, appState) => {
      ...
      // The waiver blackout holds manual sales too: a hammer entered between
      // a waiver cutoff and its published results would hand out a player the
      // engine is about to walk (docs/DESIGN-WAIVERS.md 2.3/4.4).
      {
        const blackout = await waiverBlackout(tx, appState);
        if (blackout.paused) {
          return reject(
            "sales_paused",
            `Sales are paused while ${blackout.label} resolves; they reopen when the results publish.`,
          );
        }
      }
      ...
      // Phase 1 uses the player's tier opening bid. In phase 2 every nominated
      // player opens at the league-wide minimum ($1) regardless of tier - a
      // nomination just puts the player on the block to be bid up from the floor.
      const openBid =
        appState.phase === 2
          ? minOpenBid(cfg)
          : player.tier != null
      ...
      // Waiver era: stamp the live period (id + label-as-stage) so Bid 2
      // sales file under their period. Pre-era (no current period) the legacy
      // column defaults apply unchanged.
      const stamp = await currentPeriodStamp(tx, appState);
      const [sale] = stamp
        ? await tx`
            insert into sales (player_id, manager_id, price, lot_no, phase, stage, period_id)
            values (${playerId}, ${managerId}, ${price}, ${lotNo}, ${phase}, ${stamp.stage}, ${stamp.periodId})
