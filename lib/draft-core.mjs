...
export async function recordSale(sql, cfg, { playerId, managerId, price, actor }) {
  try {
    return await withAuctionLock(sql, async (tx, appState) => {
      ...
      // derived with the exact same logic the board uses.
      ...
      // lot_no: phase 1 lots are numbered by queue position; phase-2
      // nominations (not in the queue) continue the running count.
      let lotNo;
      if (queueIdx >= 0) {
        lotNo = queueIdx + 1;
      } else {
