...
export async function buildPlayersPayload(sql, cfg) {
  const [
    ...
    // Joined to the ACTIVE sale and clipped to trades at/after it, so a
    // previous incarnation's trades cannot move a re-signed player.
    sql`
  ...
  // Original hammer buyers + prices: current ownership resolves THROUGH trades,
  // but auction spend (sunk, never refunded) stays keyed to the original buyer.
  const saleRows = soldValueRows.map((r) => ({
    playerId: r.player_id,
