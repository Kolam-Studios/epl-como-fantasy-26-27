...
export async function buildPlayersPayload(sql, cfg) {
  const [
    ...
    sql`
      select s.player_id, s.manager_id, s.price, s.lot_no, s.created_at,
  ...
  const soldById = new Map(
    soldValueRows.map((r) => [
      r.player_id,
      { managerId: r.manager_id, price: r.price, value: num(r.value), retainability: num(r.retainability) },
    ]),
  );
  ...
  const players = playerRows.map((r) => {
    ...
    const retainability = sold ? (soldById.get(r.id)?.retainability ?? null) : null;
