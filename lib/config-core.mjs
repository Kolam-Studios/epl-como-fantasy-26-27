...
export function validateConfig(cfg) {
  ...
  // budgetTopUp: OPTIONAL post-auction wallet grant. Absent = no top-up.
  if (cfg.budgetTopUp !== undefined && cfg.budgetTopUp !== null) {
    if (typeof cfg.budgetTopUp !== "number" || !Number.isInteger(cfg.budgetTopUp) || cfg.budgetTopUp < 0) {
      fail(
        "budgetTopUp must be a whole number of dollars of 0 or more " +
          `(got ${JSON.stringify(cfg.budgetTopUp)}).`,
      );
    }
  }
  ...
  // periods (waiver era): OPTIONAL - a config without periods is a valid
  // auction-only config. When present, every entry must be well-formed: a
  // malformed period must fail loudly at startup, not seed a broken season.
  const periods = cfg.periods;
  if (periods !== undefined) {
    if (!Array.isArray(periods) || periods.length === 0) {
      fail("periods, when present, must be a non-empty array of season phases.");
    }
    const seqs = new Set();
    const labels = new Set();
    for (const [i, p] of periods.entries()) {
      if (!isPlainObject(p)) fail(`periods[${i}] must be an object.`);
      if (!isPositiveInt(p.seq)) {
        fail(`periods[${i}].seq must be a positive whole number (got ${JSON.stringify(p.seq)}).`);
      }
      if (seqs.has(p.seq)) fail(`periods[${i}].seq ${p.seq} is duplicated - seq must be unique.`);
      seqs.add(p.seq);
      if (typeof p.label !== "string" || p.label.trim() === "") {
        fail(`periods[${i}].label must be a non-empty string.`);
      }
      if (labels.has(p.label)) fail(`periods[${i}].label "${p.label}" is duplicated - labels must be unique.`);
      labels.add(p.label);
      if (p.kind !== "rebid" && p.kind !== "waiver") {
        fail(`periods[${i}].kind must be "rebid" or "waiver" (got ${JSON.stringify(p.kind)}).`);
      }
      if (typeof p.season !== "string" || p.season.trim() === "") {
        fail(`periods[${i}].season must be a non-empty string (e.g. "COMO2526").`);
      }
      if (p.cutoffAt !== null && p.cutoffAt !== undefined) {
        if (typeof p.cutoffAt !== "string" || Number.isNaN(Date.parse(p.cutoffAt))) {
          fail(
            `periods[${i}].cutoffAt must be null (date TBD) or an ISO date-time string ` +
              `(got ${JSON.stringify(p.cutoffAt)}).`,
          );
        }
      }
    }
...
/**
 * @typedef {Object} ConfigPeriod
 * @property {number} seq
 * @property {string} label
 * @property {"rebid" | "waiver"} kind
 * @property {string} season
 * @property {string | null} [cutoffAt]  ISO instant; null/absent = date TBD
 */

/**
 * The season's configured periods in seq order; [] for an auction-only config
 * with no periods key (every waiver-era consumer must tolerate that).
 * @param {LeagueConfig} cfg
 * @returns {ConfigPeriod[]}
 */
export function configPeriods(cfg) {
  const raw = /** @type {Record<string, unknown>} */ (cfg).periods;
  if (!Array.isArray(raw)) return [];
  return /** @type {ConfigPeriod[]} */ ([...raw].sort((a, b) => a.seq - b.seq));
}

/**
 * The wallet a manager plays the SEASON with: the August auction budget plus
 * any post-auction top-up the owners have granted (`budgetTopUp`, a whole
 * number of dollars; absent = 0).
 *
 * `budget` keeps its original meaning - the money each manager had at the
 * hammer on auction night - so the auction record and the valuations model
 * still read the number they were built around. Everything that derives a
 * LIVE wallet (remaining, max bid, the waiver bid cap) goes through here, so a
 * top-up lands on every surface at once and is never stored on a row. Closed
 * periods are frozen snapshots and are not affected either way.
 *
 * @param {LeagueConfig} cfg
 * @returns {number}
 */
export function walletBudget(cfg) {
  const topUp = /** @type {Record<string, unknown>} */ (cfg).budgetTopUp;
  return cfg.budget + (typeof topUp === "number" ? topUp : 0);
}
...
/**
 * Tier numbers that are bid for by phase-2 nomination only, never offered in
 * phase 1 (marked `phase2Only: true` in config). Empty by default. Config-
 * driven so the phase-1/phase-2 split is never hardcoded to a tier number.
 * @param {LeagueConfig} cfg
 * @returns {number[]}
 */
export function phase2OnlyTiers(cfg) {
  return cfg.tiers.filter((t) => t.phase2Only === true).map((t) => t.tier);
}
