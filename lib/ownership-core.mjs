...
export async function loadOwnershipContext(db) {
  // WAIVER-ERA SPLIT (docs/DESIGN-WAIVERS.md 5): a dropped player's sales row
  // is kept (spend is sunk, no refunds) but marked released, and the player
  // may later be RE-SOLD, creating a second row. So:
  //   - `sales` (ownership) = ACTIVE rows only (released = false)
  //   - `spendRows` = EVERY row, because auction/waiver spend never refunds
  //   - movements are joined to the ACTIVE sale and filtered to trades made
  //     at/after it: a trade of the player's previous incarnation must not
  //     move the new owner's ownership.
  const [saleRows, spendRows, moveRows, tradeRows] = await Promise.all([
    db`
    ...
    `,
    db`
      select manager_id, price from sales
    `,
    db`
  ...
  return {
    ...
    spendRows: spendRows.map((r) => ({ managerId: r.manager_id, price: r.price })),
