...
import { squadSize, phase2OnlyTiers } from "./config-core.mjs";
...
async function managerCompleteness(tx, cfg) {
  const managers = await tx`select id, slot, short, sat_out from managers order by slot`;
  ...
  // A SAT-OUT manager (owner ruling: out for the season) reads complete here
  // regardless of their squad, so every rotation consumer skips them.
  ...
  return managers.map((m) => ({
    ...
    complete:
      m.sat_out === true ||
      deriveManager(cfg, ownership.filter((o) => o.managerId === m.id)).squadComplete,
  }));
}
...
export async function buildQueue(sql, cfg, { actor, rng = Math.random }) {
  return await withAuctionLock(sql, async (tx, appState) => {
    ...
    // tiers), stable id order inside each band before the shuffle. Phase-2-only
    // tiers (Tier 5, the $1 cheap band) are excluded from the phase-1 queue -
    // they are bid for by nomination in phase 2, never offered here. A null-tier
    // player (should not happen in a frozen pool) is kept, matching prior
    // behaviour.
    const phase2Tiers = phase2OnlyTiers(cfg);
    const players = phase2Tiers.length
      ? await tx`
          select id, tier from players
          where tier is null or tier <> all(${phase2Tiers})
          order by tier asc nulls last, id
        `
      : await tx`select id, tier from players order by tier asc nulls last, id`;
    if (players.length === 0) {
...
export async function noBid(sql, cfg, { actor }) {
  return await withAuctionLock(sql, async (tx, appState) => {
    ...
    if (phase === 1 && queue.length > 0) {
      const soldRows = await tx`select player_id from sales where released = false`;
...
export async function endPhaseOne(sql, cfg, { actor }) {
  return await withAuctionLock(sql, async (tx, appState) => {
    ...
    // Phase-2-only tiers (Tier 5) are never offered in phase 1, so they must
    // NOT count as "unoffered" here or the phase-1 -> phase-2 transition could
    // never fire. They become nominable in phase 2.
    const phase2Tiers = phase2OnlyTiers(cfg);
    const [{ n: unoffered }] = await tx`
      select count(*)::int as n
      from players p
      where not exists (select 1 from sales s where s.player_id = p.id and s.released = false)
        and not exists (select 1 from lot_events e where e.player_id = p.id)
        ${phase2Tiers.length ? tx`and (p.tier is null or p.tier <> all(${phase2Tiers}))` : tx``}
    `;
    if (unoffered > 0) {
      return reject(
        "players_unoffered",
