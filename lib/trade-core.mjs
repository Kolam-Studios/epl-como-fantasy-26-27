...
import { auctionSpendByManager, deriveManager, resolveOwnership, tradeCashByManager } from "./derive-core.mjs";
...
export async function recordTrade(sql, cfg, args) {
  ...
  return await withAuctionLock(sql, async (tx, appState) => {
    ...
    // THE BLACKOUT (docs/DESIGN-WAIVERS.md 2.3/4.4): trades pause from a
    // waiver period's cutoff until its results publish, so the engine always
    // runs on the budgets and rosters the forms were final against.
    const blackout = await waiverBlackout(tx, appState);
    if (blackout.paused) {
      return reject(
        "trades_paused",
        `Trades are paused while ${blackout.label} resolves; they reopen when the results publish.`,
      );
    }
    ...
    // Auction spend is sunk on the original buyer and unchanged by this trade.
    const auctionByManager = auctionSpendByManager(ctx.spendRows);
    ...
    const checkSide = (mid) => {
      ...
      const d = deriveManager(cfg, owned, cashAfter[mid] || 0, auctionByManager[mid] || 0);
    ...
    // Waiver era: stamp the live period (id + label-as-stage). Pre-era
    // (no current period) the legacy column defaults apply unchanged.
    const stamp = await currentPeriodStamp(tx, appState);
    const [trade] = stamp
      ? await tx`
          insert into trades (manager_a, manager_b, cash_a_to_b, cash_b_to_a, stage, period_id)
          values (${managerA}, ${managerB}, ${cashAToB}, ${cashBToA}, ${stamp.stage}, ${stamp.periodId})
